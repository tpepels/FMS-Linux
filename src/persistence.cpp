...
constexpr std::uint32_t kWrittenSectionCount = 7;
...
public:
    ...
    ~TemporaryFile() {
        if (descriptor_ >= 0) (void)::close(descriptor_);
        if (!committed_) (void)::unlink(path_.c_str());
    }
...
void writeAdvancedFmPatch(ByteWriter& writer, const AdvancedFmPatch& patch) {
    ...
    for (const AdvancedModSlot& slot : patch.modulation) writeAdvancedModSlot(writer, slot);
}
...
[[nodiscard]] AdvancedFmPatch readAdvancedFmPatch(ByteReader& reader) {
    ...
    for (AdvancedModSlot& slot : patch.modulation) slot = readAdvancedModSlot(reader);
    return patch;
}
...
[[nodiscard]] Step readStep(ByteReader& reader, std::uint16_t sectionVersion) {
    ...
    for (std::int8_t& interval : step.chord) interval = reader.getI8("chord interval");
...
void writeTrack(ByteWriter& writer, const TrackData& track) {
    for (const Step& step : track.steps) writeStep(writer, step);
...
[[nodiscard]] TrackData readTrack(ByteReader& reader, std::uint16_t sectionVersion) {
    TrackData track;
    for (Step& step : track.steps) step = readStep(reader, sectionVersion);
...
[[nodiscard]] std::vector<std::uint8_t> serializeState(const AppState& state) {
    ...
    for (const TrackData& track : state.tracks) writeTrack(tracks, track);
    ...
    for (const Step& step : state.fmPalette) writeStep(fmPalette, step);
    ...
    for (const Step& step : state.noisePalette) writeStep(noisePalette, step);
    ...
    for (const std::uint8_t button : state.controller.buttons) controller.putU8(button);
...
[[nodiscard]] AppState readVersion1Payload(const std::uint8_t* bytes, std::size_t size) {
    ...
    for (std::uint32_t sectionIndex = 0; sectionIndex < sectionCount; ++sectionIndex) {
        ...
        if (tag == kGlobalTag) {
            if (haveGlobal) throw ParseError("duplicate GLOB section");
            haveGlobal = true;
            ...
            if (haveTracks) throw ParseError("duplicate TRAK section");
            haveTracks = true;
            ...
            for (TrackData& track : decoded.tracks) track = readTrack(section, version);
            section.requireEnd();
        } else if (tag == kPatternsTag) {
            if (havePatterns) throw ParseError("duplicate PATT section");
            havePatterns = true;
            requireStepSectionVersion(version, flags, tag);
            ...
            if (haveBanks) throw ParseError("duplicate BANK section");
            haveBanks = true;
            requireSectionVersion(version, kBaseSectionVersion, flags, tag);
            ...
            if (haveFmPalette) throw ParseError("duplicate FMPA section");
            haveFmPalette = true;
            requireStepSectionVersion(version, flags, tag);
            ...
            for (Step& step : decoded.fmPalette) step = readStep(section, version);
            section.requireEnd();
        } else if (tag == kNoisePaletteTag) {
            if (haveNoisePalette) throw ParseError("duplicate NSPA section");
            ...
            for (Step& step : decoded.noisePalette) step = readStep(section, version);
            section.requireEnd();
        } else if (tag == kControllerTag) {
            if (haveController) throw ParseError("duplicate CTRL section");
            haveController = true;
...
[[nodiscard]] fs::path homeDirectory() {
    ...
    if (requestedSize < 1024) requestedSize = 16384;
...
bool saveState(const AppState& state, const std::string& path, std::string& error) {
    ...
    try {
        AppState clean = state;
        sanitize(clean);
        const std::vector<std::uint8_t> bytes = serializeState(clean);
        return writeAtomically(fs::path(path), bytes, error);
